...
- 模块化开发推荐使用`type="module"`
  ...
  **非阻塞I/O：** 非阻塞 I/O 是一种编程模式，可以让一个线程在等待 I/O 操作（如读写文件、网络请求）的同时继续处理其他任务，而不被阻塞住；
  ...
  ### 44、什么是事件总线(EventBus)？
  &emsp;&emsp;***事件总线是实现跨组件或跨模块通信的一种方式，核心作用是解耦事件的发送者和接收者；其实现原理是基于发布-订阅模式通过class类分装维护一个时间注册表，提供三个核心方法`on()/emit()/off()`；***

  ### 45、pm2是什么？有什么作用？

  &emsp;&emsp;***`pm2`是NodeJS生态最常用的进程管理器，它可以帮助NodeJS项目重启服务、开机自启、日志/监控、零停机平滑发版等；***